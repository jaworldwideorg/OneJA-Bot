...
| Microsoft             | `microsoft`             | `AUTH_MICROSOFT_ID`, `AUTH_MICROSOFT_SECRET`, `AUTH_MICROSOFT_LABEL` (optional), `AUTH_MICROSOFT_AUTHORITY_URL`, `AUTH_MICROSOFT_TENANT_ID`              |
...
AUTH_GOOGLE_LABEL="Work Account"
```

The label will be automatically prefixed with a localized prefix, so `AUTH_OKTA_LABEL="Corporate SSO"` will display as "Continue with Corporate SSO" in English or "使用 Corporate SSO 登录" in Chinese.

<Callout type={'tip'}>
  Provide only the provider name in the label (e.g., "Corporate SSO", not "Continue with Corporate SSO"). The prefix is added automatically and localized based on the user's language setting.
</Callout>
